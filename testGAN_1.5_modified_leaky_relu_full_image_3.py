...
def disc_spatial(input_low,input_high,reuse=False,batch_norm=False,train=None):
  ...
  with tf.variable_scope("disc_spatial",reuse=reuse):
    ...
    con1,_=convolutional_layer(input_merge,64,filter,tf.nn.leaky_relu,stride=stride,name="con1",reuse=reuse,batch_norm=False,train=False) # the first convolutional_layer does not use the batch_norm
    con2,_=convolutional_layer(con1,128,filter,tf.nn.leaky_relu,stride=stride,name="con2",reuse=reuse,batch_norm=batch_norm,train=train)
    con3,_=convolutional_layer(con2,256,filter,tf.nn.leaky_relu,stride=stride,name="con3",reuse=reuse,batch_norm=batch_norm,train=train)

    #here in tempoGAN, we use the stride=[1] for the last layer!!!
    con4,_=convolutional_layer(con3,512,filter,tf.nn.leaky_relu,stride=[1],name="con4",reuse=reuse,batch_norm=batch_norm,train=train)
...
def disc_tempo(input_high,num_tempo_channels=3,reuse=False,batch_norm=False,train=None):
  with tf.variable_scope("disc_tempo",reuse=reuse):
    ...
    con1,_=convolutional_layer(input_high,64,filter,tf.nn.leaky_relu,stride=stride,name="con1",reuse=reuse,batch_norm=False,train=False) # the first convolutional_layer does not use the batch_norm
    con2,_=convolutional_layer(con1,128,filter,tf.nn.leaky_relu,stride=stride,name="con2",reuse=reuse,batch_norm=batch_norm,train=train)
    con3,_=convolutional_layer(con2,256,filter,tf.nn.leaky_relu,stride=stride,name="con3",reuse=reuse,batch_norm=batch_norm,train=train)
...
with  tf.device(device_str):
   ...
   # saved_model_number=0
    ...
    if(not get_model):
      ...
      #test here!
      batch_xs_test=get_input(0,62,1,"low")
      batch_ys_test=get_input(0,62,1,"high")
      batch_xs_test=batch_xs_test.reshape(batch_size,size_tile_low,size_tile_low,1)
      if(use_velocities):
        batch_vxs_test=get_input_vx(0,62,1,"low")
        batch_vys_test=get_input_vy(0,62,1,"low")
      ...
      k=start_frame
      now=k*help_num+2
      batch_xs_train=get_input(0,now,1,"low")
      batch_ys_train=get_input(0,now,1,"high")
      k=k+1
      while(k<end_frame):
        now=k*help_num+2
        batch_xs_train_add=get_input(0,now,1,"low")
        batch_ys_train_add=get_input(0,now,1,"high")
        batch_xs_train=np.concatenate((batch_xs_train,batch_xs_train_add),0)
        batch_ys_train=np.concatenate((batch_ys_train,batch_ys_train_add),0)
        k=k+1

      batch_xs_train=batch_xs_train.reshape(batch_size*(end_frame-start_frame),size_tile_low,size_tile_low,1)
      batch_ys_train=batch_ys_train.reshape(batch_size*(end_frame-start_frame),-1)

      if(use_velocities):
        k=start_frame
        now=k*help_num+2
        batch_vxs_train=get_input_vx(0,now,1,"low")
        batch_vys_train=get_input_vy(0,now,1,"low")

        k=k+1
        while(k<end_frame):
          now=k*help_num+2
          batch_vxs_train_add=get_input_vx(0,now,1,"low")
          batch_vys_train_add=get_input_vy(0,now,1,"low")
          batch_vxs_train=np.concatenate((batch_vxs_train,batch_vxs_train_add),0)
          batch_vys_train=np.concatenate((batch_vys_train,batch_vys_train_add),0)
          k=k+1

        batch_vxs_train=batch_vxs_train.reshape(batch_size*(end_frame-start_frame),size_tile_low,size_tile_low,1)
        batch_vys_train=batch_vys_train.reshape(batch_size*(end_frame-start_frame),size_tile_low,size_tile_low,1)
        batch_xs_train=np.concatenate((batch_xs_train,batch_vxs_train,batch_vys_train),-1)

      batch_xs_train=batch_xs_train.reshape(batch_size*(end_frame-start_frame),-1)


      k=start_frame
      now=k*help_num+1
      batch_xts_train=get_input(0,now,3,"low")
      batch_yts_train=get_input(0,now,3,"high")
      k=k+1

      while(k<end_frame):
        now=k*help_num+1
        batch_xts_train_add=get_input(0,now,3,"low")
        batch_yts_train_add=get_input(0,now,3,"high")
        batch_xts_train=np.concatenate((batch_xts_train,batch_xts_train_add),0)
        batch_yts_train=np.concatenate((batch_yts_train,batch_yts_train_add),0)
        k=k+1

      batch_xts_train=batch_xts_train.reshape(batch_size*(end_frame-start_frame),n_t,size_tile_low,size_tile_low,1)
      batch_yts_train=batch_yts_train.reshape(batch_size*(end_frame-start_frame),n_t,-1)
      batch_yts_train=batch_yts_train.transpose((0,2,1))

      if(use_velocities):
        k=start_frame
        now=k*help_num+1
        batch_vxts_train=get_input_vx(0,now,3,"low")
        batch_vyts_train=get_input_vy(0,now,3,"low")
        k=k+1
        while(k<end_frame):
          now=k*help_num+1
          batch_vxts_train_add=get_input_vx(0,now,3,"low")
          batch_vyts_train_add=get_input_vy(0,now,3,"low")
          batch_vxts_train=np.concatenate((batch_vxts_train,batch_vxts_train_add),0)
          batch_vyts_train=np.concatenate((batch_vyts_train,batch_vyts_train_add),0)
          k=k+1

        batch_vxts_train=batch_vxts_train.reshape(batch_size*(end_frame-start_frame),n_t,size_tile_low,size_tile_low,1)
        batch_vyts_train=batch_vyts_train.reshape(batch_size*(end_frame-start_frame),n_t,size_tile_low,size_tile_low,1)
        batch_xts_train=np.concatenate((batch_xts_train,batch_vxts_train,batch_vyts_train),-1)

      batch_xts_train=batch_xts_train.reshape(batch_size*(end_frame-start_frame)*n_t,-1)
        ...
        for i in range(train_index_begin,train_index_end):
          ...
          while(j<frame_num_per_sim):
            ...
            for runs in range(disc_spatial_runs):              
              dict_train={x:batch_xs_train,x_disc_spatial:batch_xs_train,y:batch_ys_train,train:True,lr_global_step:mark}
            ...
            for runs in range(disc_tempo_runs):                            
              dict_train={x_t:batch_xts_train,y_t:batch_yts_train,train:True,lr_global_step:mark}
            ...
            for runs in range(gen_runs):
              dict_train={x:batch_xs_train,x_disc_spatial:batch_xs_train,y:batch_ys_train,train:True,lr_global_step:mark,x_t:batch_xts_train,k_k_tempo:k_tempo,k_k2:k2,k_k1:k1}
