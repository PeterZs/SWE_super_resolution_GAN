...
print >> file_name,"start_frame: %d"%start_frame
print >> file_name,"end_frame: %d"%end_frame
...
batch_size=(size_sim_low/size_tile_low)**2
batch_norm=1
...
with  tf.device(device_str):
    ...
    if(test_mode):
        ...
        if(gradient_loss_kind==0):
          gen_loss=gen_l2_loss+gen_gradient_loss

        elif(gradient_loss_kind==1):
          gen_loss=gen_l2_loss+gen_gradient_l2_loss

        elif(gradient_loss_kind==2):
          gen_loss=gen_l2_loss+gen_gradient_loss_no_abs

        elif(gradient_loss_kind==3):
          gen_loss=gen_l2_loss+gen_gradient_l2_loss_no_abs
        elif(gradient_loss_kind==4):
          gen_loss=gen_gradient_loss

        elif(gradient_loss_kind==5):
          gen_loss=gen_gradient_l2_loss

        elif(gradient_loss_kind==6):
          gen_loss=gen_gradient_loss_no_abs

        elif(gradient_loss_kind==7):
          gen_loss=gen_gradient_l2_loss_no_abs
        ...
        with tf.control_dependencies(disc_tempo_update_ops):
          disc_tempo_optimizer=tf.train.AdamOptimizer(learning_rate,beta1=beta)
          gradient_disc_tempo=disc_tempo_optimizer.compute_gradients(disc_tempo_loss,var_list=disc_tempo_variable)
          gradient_disc_tempo_holder=[(tf.placeholder(tf.float32,shape=g.get_shape()),v) for (g,v) in gradient_disc_tempo]

         # print("gradient_disc_tempo_holder",gradient_disc_tempo_holder)
          train_disc_tempo_op=disc_tempo_optimizer.apply_gradients(gradient_disc_tempo_holder)

        with tf.control_dependencies(disc_spatial_update_ops):
          disc_spatial_optimizer=tf.train.AdamOptimizer(learning_rate,beta1=beta)
          gradient_disc_spatial=disc_spatial_optimizer.compute_gradients(disc_spatial_loss,var_list=disc_spatial_variable)
          gradient_disc_spatial_holder=[(tf.placeholder(tf.float32,shape=g.get_shape()),v) for (g,v) in gradient_disc_spatial]

         # print("gradient_disc_spatial_holder",gradient_disc_spatial_holder)
          train_disc_spatial_op=disc_spatial_optimizer.apply_gradients(gradient_disc_spatial_holder)          

        with tf.control_dependencies(gen_update_ops):
          gen_optimizer=tf.train.AdamOptimizer(learning_rate,beta1=beta)
          gradient_gen=gen_optimizer.compute_gradients(gen_loss,var_list=gen_variable)
          gradient_gen_holder=[(tf.placeholder(tf.float32,shape=g.get_shape()),v) for (g,v) in gradient_gen]

         # print("gradient_gen_holder",gradient_gen_holder)
          train_gen_op=gen_optimizer.apply_gradients(gradient_gen_holder)
   ...
   # saved_model_number=0
    ...
    if(not get_model):
      ...
      #test here!
      batch_xs_test=get_input(0,64,1,"low")
      batch_ys_test=get_input(0,64,1,"high")
      batch_xs_test=batch_xs_test.reshape(batch_size,size_tile_low,size_tile_low,1)
      if(use_velocities):
        batch_vxs_test=get_input_vx(0,64,1,"low")
        batch_vys_test=get_input_vy(0,64,1,"low")
        batch_vxs_test=batch_vxs_test.reshape(batch_size,size_tile_low,size_tile_low,1)
        batch_vys_test=batch_vys_test.reshape(batch_size,size_tile_low,size_tile_low,1)
        batch_xs_test=np.concatenate((batch_xs_test,batch_vxs_test,batch_vys_test),-1)
      batch_xs_test=batch_xs_test.reshape(batch_size,-1)
      ...
      for k in range(start_frame,end_frame):
        now=k*help_num+4
        batch_xs_train_k=get_input(0,now,1,"low")
        batch_ys_train_k=get_input(0,now,1,"high")
        batch_xs_train_k=batch_xs_train_k.reshape(batch_size,size_tile_low,size_tile_low,1)
        batch_ys_train_k=batch_ys_train_k.reshape(batch_size,-1)

        if(use_velocities):
          batch_vxs_train_k=get_input_vx(0,now,1,"low")
          batch_vys_train_k=get_input_vy(0,now,1,"low")

          batch_vxs_train_k=batch_vxs_train_k.reshape(batch_size,size_tile_low,size_tile_low,1)
          batch_vys_train_k=batch_vys_train_k.reshape(batch_size,size_tile_low,size_tile_low,1)
          
          batch_xs_train_k=np.concatenate((batch_xs_train_k,batch_vxs_train_k,batch_vys_train_k),-1)

        batch_xs_train_k=batch_xs_train_k.reshape(batch_size,-1)

        batch_xs_train.append(batch_xs_train_k)
        batch_ys_train.append(batch_ys_train_k)        

      
      for k in range (start_frame,end_frame):
        now=k*help_num+3
        batch_xts_train_k=get_input(0,now,3,"low")
        batch_yts_train_k=get_input(0,now,3,"high")

        batch_xts_train_k=batch_xts_train_k.reshape(batch_size,n_t,size_tile_low,size_tile_low,1)
        batch_yts_train_k=batch_yts_train_k.reshape(batch_size,n_t,-1)
        batch_yts_train_k=batch_yts_train_k.transpose((0,2,1))

        if(use_velocities):        
          batch_vxts_train_k=get_input_vx(0,now,3,"low")
          batch_vyts_train_k=get_input_vy(0,now,3,"low")
          batch_vxts_train_k=batch_vxts_train_k.reshape(batch_size,n_t,size_tile_low,size_tile_low,1)
          batch_vyts_train_k=batch_vyts_train_k.reshape(batch_size,n_t,size_tile_low,size_tile_low,1)
          batch_xts_train_k=np.concatenate((batch_xts_train_k,batch_vxts_train_k,batch_vyts_train_k),-1)

        batch_xts_train_k=batch_xts_train_k.reshape(batch_size*n_t,-1)

        batch_xts_train.append(batch_xts_train_k)
        batch_yts_train.append(batch_yts_train_k)
        
      for epoch in  range(epochs):
        
        dict_train={x:batch_xs_test,x_disc_spatial:batch_xs_test,train:False}
        ...
        for i in range(train_index_begin,train_index_end):
          ...
          while(j<frame_num_per_sim):
            ...
            for runs in range(disc_spatial_runs):              
              ...
              e_s=end_frame-start_frame
              for k in range(e_s):

                dict_train={x:batch_xs_train[k],x_disc_spatial:batch_xs_train[k],y:batch_ys_train[k],train:True,lr_global_step:mark}                
                grad_k,loss_k=sess.run([gradient_disc_spatial,disc_spatial_loss],feed_dict=dict_train,options=run_options,run_metadata=run_metadata)

                grads.append(grad_k)
                losses.append(loss_k)

              disc_spatial_loss_return=sum([loss for loss in losses])
              grads_sum={}
              for k in range(len(gradient_disc_spatial_holder)):
                key=gradient_disc_spatial_holder[k][0]
                grads_sum[key]=sum([g[k][0] for g in grads])

              #print("len of dict_train disc_spatial",len(dict_train))
              dict_train.update(grads_sum)
              _=sess.run([train_disc_spatial_op],feed_dict=dict_train,options=run_options,run_metadata=run_metadata)
            ...
            for runs in range(disc_tempo_runs):              
              ...
              for k in range(e_s):
                ...
                dict_train={x_t:batch_xts_train[k],y_t:batch_yts_train[k],train:True,lr_global_step:mark}                
                grad_k,loss_k=sess.run([gradient_disc_tempo,disc_tempo_loss],feed_dict=dict_train,options=run_options,run_metadata=run_metadata)

                grads.append(grad_k)
                losses.append(loss_k)

              disc_tempo_loss_return=sum([loss for loss in losses])
              grads_sum={}
              for k in range(len(gradient_disc_tempo_holder)):
                key=gradient_disc_tempo_holder[k][0]
                grads_sum[key]=sum([g[k][0] for g in grads])

              #print("len of dict_train disc_tempo",len(dict_train))
              dict_train.update(grads_sum)
              _=sess.run([train_disc_tempo_op],feed_dict=dict_train,options=run_options,run_metadata=run_metadata)
            ...
            for runs in range(gen_runs):
              ...
              for k in range(e_s):
                ...
                dict_train={x:batch_xs_train[k],x_disc_spatial:batch_xs_train[k],y:batch_ys_train[k],train:True,lr_global_step:mark,x_t:batch_xts_train[k],k_k_tempo:k_tempo,k_k2:k2,k_k1:k1}                
                grad_k,loss_k=sess.run([gradient_gen,gen_loss],feed_dict=dict_train,options=run_options,run_metadata=run_metadata)

                grads.append(grad_k)
                losses.append(loss_k)

              gen_loss_return=sum([loss for loss in losses])
              grads_sum={}
              for k in range(len(gradient_gen_holder)):
                key=gradient_gen_holder[k][0]
                grads_sum[key]=sum([g[k][0] for g in grads])

             # print("len of dict_train gen",len(dict_train))
              dict_train.update(grads_sum)
              _=sess.run([train_gen_op],feed_dict=dict_train,options=run_options,run_metadata=run_metadata)
