...
def residual_block(input_layer,output_channels_a,output_channels_b,reuse,batch_norm,filter_size=3):
  ...
  gc1,_=convolutional_layer(input_layer,output_channels_a,filter_a,tf.nn.relu,stride=[1],name="gcA%d"%residual_block_id,batch_norm=batch_norm,train=train,reuse=reuse)
  ...
  res_layer=tf.nn.relu(tf.add(gc2,gs1))
  residual_block_id+=1
  return res_layer
