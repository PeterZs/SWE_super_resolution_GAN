        ...
        for i in range(train_index_begin,train_index_end):
          ...
          while(j<frame_num_per_sim):
            ...
            for runs in range(disc_spatial_runs):
              k=start_frame
              now=k*help_num+2
              batch_xs=get_input(0,now,1,"low")
              batch_ys=get_input(0,now,1,"high")
              k=k+1
              while(k<end_frame):
                now=k*help_num+2
                batch_xs_add=get_input(0,now,1,"low")
                batch_ys_add=get_input(0,now,1,"high")
                batch_xs=np.concatenate((batch_xs,batch_xs_add),0)
                batch_ys=np.concatenate((batch_ys,batch_ys_add),0)
                k=k+1
                
              batch_xs=batch_xs.reshape(batch_size*(end_frame-start_frame),size_tile_low,size_tile_low,1)
              batch_ys=batch_ys.reshape(batch_size*(end_frame-start_frame),-1)

              if(use_velocities):

                k=start_frame
                now=k*help_num+2
                batch_vxs=get_input_vx(0,now,1,"low")
                batch_vys=get_input_vy(0,now,1,"low")

                k=k+1
                while(k<end_frame):
                  now=k*help_num+2
                  batch_vxs_add=get_input_vx(0,now,1,"low")
                  batch_vys_add=get_input_vy(0,now,1,"low")
                  batch_vxs=np.concatenate((batch_vxs,batch_vxs_add),0)
                  batch_vys=np.concatenate((batch_vys,batch_vys_add),0)
                  k=k+1
                  
                batch_vxs=batch_vxs.reshape(batch_size*(end_frame-start_frame),size_tile_low,size_tile_low,1)
                batch_vys=batch_vys.reshape(batch_size*(end_frame-start_frame),size_tile_low,size_tile_low,1)                
                batch_xs=np.concatenate((batch_xs,batch_vxs,batch_vys),-1)

              batch_xs=batch_xs.reshape(batch_size*(end_frame-start_frame),-1)
            ...
            for runs in range(disc_tempo_runs):
              ...
              k=start_frame
              now=k*help_num+1
              batch_xts=get_input(0,now,3,"low")
              batch_yts=get_input(0,now,3,"high")
              k=k+1

              while(k<end_frame):
                now=k*help_num+1
                batch_xts_add=get_input(0,now,3,"low")
                batch_yts_add=get_input(0,now,3,"high")
                batch_xts=np.concatenate((batch_xts,batch_xts_add),0)
                batch_yts=np.concatenate((batch_yts,batch_yts_add),0)
                k=k+1
                            
              batch_xts=batch_xts.reshape(batch_size*(end_frame-start_frame),n_t,size_tile_low,size_tile_low,1)              
              batch_yts=batch_yts.reshape(batch_size*(end_frame-start_frame),n_t,-1)
              ...
              if(use_velocities):
                ...
                k=start_frame
                now=k*help_num+1
                batch_vxts=get_input_vx(0,now,3,"low")
                batch_vyts=get_input_vy(0,now,3,"low")
                k=k+1
                while(k<end_frame):
                  now=k*help_num+1
                  batch_vxts_add=get_input_vx(0,now,3,"low")
                  batch_vyts_add=get_input_vy(0,now,3,"low")
                  batch_vxts=np.concatenate((batch_vxts,batch_vxts_add),0)
                  batch_vyts=np.concatenate((batch_vyts,batch_vyts_add),0)
                  k=k+1
                  
                batch_vxts=batch_vxts.reshape(batch_size*(end_frame-start_frame),n_t,size_tile_low,size_tile_low,1)
                batch_vyts=batch_vyts.reshape(batch_size*(end_frame-start_frame),n_t,size_tile_low,size_tile_low,1)
                
                batch_xts=np.concatenate((batch_xts,batch_vxts,batch_vyts),-1)

              batch_xts=batch_xts.reshape(batch_size*(end_frame-start_frame)*n_t,-1)
            ...
            for runs in range(gen_runs):
              ...
              batch_xs=get_input(0,now,1,"low")
              batch_ys=get_input(0,now,1,"high")              
              batch_xts=get_input(0,now-1,3,"low")
              ...
              while(k<end_frame):
                ...
                batch_xs_add=get_input(0,now,1,"low")
                batch_ys_add=get_input(0,now,1,"high")
                batch_xts_add=get_input(0,now-1,3,"low")
                batch_xs=np.concatenate((batch_xs,batch_xs_add),0)
                batch_ys=np.concatenate((batch_ys,batch_ys_add),0)
                batch_xts=np.concatenate((batch_xts,batch_xts_add),0)
                k=k+1
